...
def restock_stock(book):
  if book.restock(amount.get()):
    ...
    action_feedback.set("hey ! Total of {} restocked into {}".format(amount.get(), book.name))
    
  else:
    action_feedback.set("hey! enter a positive number")

# Create the sell function
def sell_stock(book):
  if book.sell(amount.get()):
    message_text.set(sell_message)
    action_feedback.set("hey ! Total of {} sold from {}".format(amount.get(), book.name))
    
  else:
    action_feedback.set("Not enough stock in {} or not a valid amount, It will come on error".format(book.name))

def quicksell_stock(book):
  if book.quicksell(amount.get()):
    message_text.set(sell_message)
    action_feedback.set("hey ! Total of {} sold from {}".format(amount.get(), book.name))
...
def manage_action01():
  try:
    for book in book_list:
      if chosen_book.get() == book.name:
        if chosen_action.get() == "quicksell":
            quicksell_stock(book)
        else:
            quicksell_stock(book)
...
# Set up Lists
book_list = []
restock_message = ("[ you are Restocking ]!")
sell_message = ("[ you on aSale made ]!")
...
message_text.set(" Oscar's root can help u restock, sell and view the tolal stock in here :")
...
book_label = ttk.Label(bottom_frame, text="[ Comic's Book ]: ")
...
submit_button.grid(row=10, column=10, columnspan=6, padx=10, pady=10)

quick_button = ttk.Button(bottom_frame, text="[ quick sell ]", command=manage_action01)
quick_button.grid(row=9, column=10, columnspan=6, padx=10, pady=10)
